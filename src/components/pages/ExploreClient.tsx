...
function ExplorePageContent() {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      ...
      // Update URL query parameters for shareable search & filter links
            ...
            <div className="space-y-2">
              ...
              <div className="space-y-1.5">
                <button
                  ...
                  className={`w-full text-left text-xs py-2 px-3 rounded-lg transition-all duration-300 flex items-center justify-between ${
                    selectedCategory === "" ? "bg-gold-champagne/15 text-gold-champagne font-semibold border-l-2 border-gold-champagne shadow-sm shadow-gold-champagne/10" : "hover:bg-white/5 text-ivory/80"
                  }`}
                >
                  <span>All Categories</span>
                  <span className="text-[9px] font-mono opacity-60">All</span>
                </button>
                {MOCK_CATEGORIES.map((cat) => (
                  <button
                    ...
                    className={`w-full text-left text-xs py-2 px-3 rounded-lg transition-all duration-300 flex items-center justify-between ${
                      selectedCategory === cat.slug ? "bg-gold-champagne/15 text-gold-champagne font-semibold border-l-2 border-gold-champagne shadow-sm shadow-gold-champagne/10" : "hover:bg-white/5 text-ivory/80"
                    }`}
                  >
                    <span>{cat.name}</span>
            ...
            <div className="space-y-2">
              ...
              <div className="space-y-1.5">
                <button
                  ...
                  className={`w-full text-left text-xs py-2 px-3 rounded-lg transition-all duration-300 ${
                    selectedBrand === "" ? "bg-gold-champagne/15 text-gold-champagne font-semibold border-l-2 border-gold-champagne" : "hover:bg-white/5 text-ivory/80"
                  }`}
                >
                  All Brands
                </button>
                {MOCK_BRANDS.map((b) => (
                  <button
                    ...
                    className={`w-full text-left text-xs py-2 px-3 rounded-lg transition-all duration-300 ${
                      selectedBrand === b.slug ? "bg-gold-champagne/15 text-gold-champagne font-semibold border-l-2 border-gold-champagne" : "hover:bg-white/5 text-ivory/80"
                    }`}
                  >
                    {b.name}
                  </button>
              ...
              <div className="grid grid-cols-2 gap-2">
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-white/5 border border-white/10 text-[10px] rounded p-1.5 focus:outline-none focus:border-gold-champagne/50"
                  />
                </div>
                <div>
                    ...
                    <div className="h-48 overflow-hidden relative bg-black/40 flex items-center justify-center border-b border-white/5">
                      ...
                      <div className="absolute top-3 left-3 right-3 flex justify-between z-20">
                        <div className="flex items-center gap-1.5">
                          ...
                          <span className={`text-[8px] font-semibold uppercase tracking-wider px-2 py-0.5 rounded backdrop-blur border flex items-center gap-1 ${
                            isAvailable ? "bg-emerald-950/80 text-emerald-300 border-emerald-500/30" : "bg-rose-950/80 text-rose-300 border-rose-500/30"
                          }`}>
                            <span className={`w-1 h-1 rounded-full ${isAvailable ? "bg-emerald-400 animate-pulse" : "bg-rose-400"}`} />
                            {isAvailable ? "In Stock" : "Booked"}
                          </span>
                        </div>
                    ...
                    <div className="p-5 flex-1 flex flex-col justify-between">
                      <div className="space-y-2">
                        <div className="flex items-center justify-between text-[10px] text-muted-gray uppercase tracking-widest font-mono">
                          ...
                          <span className="text-gold-champagne/80">{MOCK_CATEGORIES.find((c) => c.id === product.categoryId)?.name}</span>
                        </div>
                        
                        <Link href={`/gear/${product.slug}`} className="block">
                          <h3 className="serif-heading text-lg font-light text-ivory group-hover:text-gold-champagne transition-colors duration-300 leading-snug">
                            {product.name}
                          </h3>
                        </Link>
                      ...
                      <div className="pt-4 space-y-3">
                        ...
                        <div className="flex justify-between items-end border-t border-white/5 pt-3">
                          <div>
                            ...
                            <span className="text-base font-semibold text-gold-champagne text-gold-glow">₹{product.dailyPrice.toLocaleString("en-IN")}</span>
                          </div>
                          <div className="text-right">
                            <span className="text-[8px] text-muted-gray uppercase block font-mono">Weekly Special</span>
                            <span className="text-xs text-ivory/80 font-mono">₹{product.weeklyPrice.toLocaleString("en-IN")}</span>
                          </div>
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-2">
                          <button
                            ...
                            className={`py-2 text-[9px] font-semibold uppercase tracking-wider rounded-lg border transition flex items-center justify-center gap-1 cursor-pointer ${
                              isCompared 
                                ? "bg-gold-champagne/15 border-gold-champagne text-gold-champagne" 
      ...
      {quickViewProduct && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative glass-panel-gold rounded-lg max-w-2xl w-full border-gold-border p-6 md:p-8 z-10 flex flex-col md:flex-row gap-6 shadow-2xl animate-fade-in">
            ...
            <div className="md:w-1/2 h-52 md:h-auto overflow-hidden rounded bg-black/45 flex items-center justify-center border border-white/5 relative">
              <Image
              ...
              <div className="flex gap-3">
                {compareList.map((product) => (
                  <div key={product.id} className="flex items-center gap-2.5 bg-white/5 border border-white/10 rounded-full pl-2.5 pr-3 py-1 text-xs">
                    <Image src={product.imagePrimary} width={20} height={20} className="w-5 h-5 rounded-full object-cover" alt={product.name} />
                    <span className="text-ivory font-medium truncate max-w-30">{product.name}</span>
                    <button
                      onClick={() => toggleCompare(product)}
