...
export default function AdminInventoryPage() {
  ...
  const handleToggleMaintenance = async (id: string, currentStatus: string) => {
    ...
    setItems((prev) =>
      prev.map((i) => (i.id === id ? { ...i, status: targetStatus.toUpperCase() as any } : i))
    );
    await adminApiClient.inventory.update(id, { status: targetStatus });
  };
    ...
    <div className="space-y-8 pb-12">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-white/5 pb-6">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-xs text-[#9a9995]">• Real-Time Single Source of Truth</span>
          </div>
          <h1 className="text-2xl md:text-3xl font-light text-[#f5f1e8] font-serif">
            Camera &amp; Lens <span className="text-[#d8b36a]">Inventory</span>
          </h1>
          <p className="text-xs text-[#9a9995] mt-1 font-light">
            Monitor serial numbers, availability state, physical condition, and maintenance logs.
          </p>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <button className="flex items-center gap-2 px-4 py-2 rounded-xl bg-[#d8b36a] text-[#070707] font-semibold text-xs shadow-md shadow-[#d8b36a]/20 hover:bg-[#e2c17c] transition">
            <Plus size={16} />
            <span>Add Unit</span>
          </button>
        </div>
      ...
      <div className="flex flex-col md:flex-row items-center justify-between gap-4 bg-[#121212] border border-white/10 p-4 rounded-2xl">
        <div className="relative w-full md:w-80">
          ...
          <input
            type="text"
            placeholder="Search equipment, serial number, brand..."
            value={search}
        ...
        <div className="flex items-center gap-3 w-full md:w-auto justify-between md:justify-end">
          ...
          <div className="flex items-center bg-[#070707] border border-white/10 rounded-lg p-1">
            <button
              onClick={() => setViewMode("grid")}
              className={`p-1.5 rounded-md ${viewMode === "grid" ? "bg-white/10 text-[#d8b36a]" : "text-[#9a9995]"}`}
            >
              <LayoutGrid size={14} />
            </button>
            <button
              onClick={() => setViewMode("table")}
              className={`p-1.5 rounded-md ${viewMode === "table" ? "bg-white/10 text-[#d8b36a]" : "text-[#9a9995]"}`}
            >
              <List size={14} />
            </button>
          </div>
          ...
          <div key={item.id} className="admin-card p-5 rounded-2xl space-y-4 admin-card-hover">
            ...
            <div className="flex items-center justify-between pt-2 border-t border-white/5 text-xs font-mono">
              ...
              <button
                ...
                className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg border text-xs font-sans transition ${
              ...
              >
                <Wrench size={12} />
                <span>{item.status === "MAINTENANCE" ? "Mark Available" : "Maintenance"}</span>
              </button>
            </div>
          </div>
        ))}
